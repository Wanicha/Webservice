<?php
	...
	if($strMode == "LOGIN"){
		...
		if(mysql_num_rows($result) != 0){
			...
			echo "<form name=frmMain id=frmMain>Login : <input size=15 type=text name=login id=login maxlength=15> Password : <input size=15 type=password name=password id=password maxlength=15> <input type=submit name=submit value=เข้าสู่ระบบ OnClick=\"return check_login(frmMain.login.value, frmMain.password.value, 'LOGIN')\"/>
			</form>ชื่อ หรือ รหัสผ่าน ไม่ถูกต้อง";
			exit();
		}
	}
